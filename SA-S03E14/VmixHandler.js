//******************************************************
//    Collection of functions to work with vMix
//
//******************************************************
...
async function InitAnswersTitle(vMix_URI, input, numberOfSlotsPerRow)
{
    ...
    for (let indexOuter = 0; indexOuter < responseFields.length; indexOuter++)
    {
        ...
        // Hide the CHECK MARK overlay
        titleUpdateUrl = vMix_URI + '/?Function=SetImageVisibleOff&Input=' + input + '&SelectedName=CHECK' + currentField + '.Source';
...
//************************************************************************
// FUNCTION: ShowCorrectCheck(vMix_URI, input, correctAnswer)
//------------------------------------------------------------------------
//    This displays a check next to the correct answer
//    (We turn them on when we write out an entry)
//************************************************************************

async function ShowCorrectCheck(vMix_URI, input, correctAnswer)
{
    const FETCH_SETTINGS = { method: "Get" };
    
    let titleUpdateUrl = null;
    let correctExtension = correctAnswer.replace('#', '');

    // Show Check Mark on Right Answer
    titleUpdateUrl = vMix_URI + '/?Function=SetImageVisibleOn&Input=' + input + '&SelectedName=CHECK' + correctExtension + '.Source';
    await fetch(titleUpdateUrl, FETCH_SETTINGS);
...
//*****************************************************************************
// FUNCTION: UpdateLeaderBoardTitle(vMix_URI, input, scoreData)
//-----------------------------------------------------------------------------
//    This updates the vMix Title showing everyone's rank on the Leader Board
//*****************************************************************************

async function UpdateLeaderBoardTitle(vMix_URI, input, scoreData)
{
    const FETCH_SETTINGS = { method: "Get" };
    ...
    for (index = 0; index < 10; index++)
    {
        ...
        // Fill in the PLACE
        titleUpdateUrl = vMix_URI + '/?Function=SetText&Input=' + input + '&SelectedName=PLACE' + (index + 1) + '.Text&value=' + encodeURIComponent(index+1);
        await fetch(titleUpdateUrl, FETCH_SETTINGS);

        // Fill in the NAME
        titleUpdateUrl = vMix_URI + '/?Function=SetText&Input=' + input + '&SelectedName=NAME' + (index + 1) + '.Text&value=' + encodeURIComponent(scoreData[index]?.name);
        await fetch(titleUpdateUrl, FETCH_SETTINGS);

        // Fill in the Score
        titleUpdateUrl = vMix_URI + '/?Function=SetText&Input=' + input + '&SelectedName=SCORE' + (index + 1) + '.Text&value=' + encodeURIComponent(scoreData[index]?.score);
        await fetch(titleUpdateUrl, FETCH_SETTINGS);
                
        // Fill in the AVATAR
        titleUpdateUrl = vMix_URI + '/?Function=SetImage&Input=' + input + '&SelectedName=AVATAR' + (index + 1) + '.Source&value=' + encodeURIComponent(scoreData[index]?.avatarURI);
        await fetch(titleUpdateUrl, FETCH_SETTINGS);
...
//************************************************************************
// FUNCTION: UpdateQuestionTitle(vMix_URI, questionObject, input)
//------------------------------------------------------------------------
//    Fills in all of the Question Related Data in the specified title
//    (All titles share the same element names for questions/options)
//************************************************************************
...
async function UpdateQuestionTitle(vMix_URI, questionObject, input)
{
    ...
    for (let indexOuter = 0; indexOuter < responseFields.length; indexOuter++)  // Loop for each on the the answers
    {
        let currentField = responseFields[indexOuter];                          // Assign the current Answer field
...
module.exports.UpdateLeaderBoardTitle = UpdateLeaderBoardTitle;
module.exports.ShowCorrectCheck = ShowCorrectCheck;
