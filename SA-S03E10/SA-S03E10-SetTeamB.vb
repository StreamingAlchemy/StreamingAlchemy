...
dim GFX1Input as Integer = 3  		'The vMix Input Number of the First GFX Title
	...
	dim TeamBRosterData As New XmlDataDocument()
	dim TeamBRosterFileStream As New Filestream(TeamRosterFolder+"\TeamRoster.xml", FileMode.Open, FileAccess.Read)

	
	' Load the file with Team B roster and build Node List
	dim TeamBRosterNodeList As XMLNodeList

	TeamBRosterData.Load(TeamBRosterFileStream)
	TeamBRosterFileStream.Close()

	TeamBRosterNodeList = TeamBRosterData.selectSingleNode("/Roster/TeamB").ChildNodes

	' Check if any players are found for Team B 
	If ( TeamBRosterNodeList Is Nothing ) Then 
	...
	PlayerXMLNode = TeamBRosterData.SelectSingleNode("/Roster/TeamB/Player[@number='" + PlayerNumber + "']")
	...
	Else
        ...
        PlayerFirstName = TeamBRosterData.SelectSingleNode("/Roster/TeamB/Player[@number='" + PlayerNumber + "']/FirstName").InnerText
		PlayerLastName = TeamBRosterData.SelectSingleNode("/Roster/TeamB/Player[@number='" + PlayerNumber + "']/LastName").InnerText
		PlayerPosition = TeamBRosterData.SelectSingleNode("/Roster/TeamB/Player[@number='" + PlayerNumber + "']/Position").InnerText
		PlayerImageURI = TeamBRosterData.SelectSingleNode("/Roster/TeamB/Player[@number='" + PlayerNumber + "']/ImageURI").InnerText
        PlayerAge = TeamBRosterData.SelectSingleNode("/Roster/TeamB/Player[@number='" + PlayerNumber + "']/Age").InnerText
		PlayerHeight = TeamBRosterData.SelectSingleNode("/Roster/TeamB/Player[@number='" + PlayerNumber + "']/Height").InnerText
		PlayerWeight = TeamBRosterData.SelectSingleNode("/Roster/TeamB/Player[@number='" + PlayerNumber + "']/Weight").InnerText
	End If
	...
	' Display Team B Jersey and hide Team A Jersey
	API.Function("SetImageVisibleOff", input:=GFX1Input, SelectedName:="JERSEYA.Source")
	API.Function("SetImageVisibleOn", input:=GFX1Input, SelectedName:="JERSEYB.Source")

	' Set the current Team B player to this Player Number
